...
where TEntityId : class
{
    ...
    )
    {
        ...
        if (string.IsNullOrEmpty(orderBy)) records = await queryable.Skip(skipAmount).Take(pageSize).ToListAsync();
        else
        {
        ...
        var totalNumberOfPages = (totalNumberOfRecords / pageSize) + (mod == 0 ? 0 : 1);
